...
router.get('/', optionalAuthMiddleware, (req, res) => {
  ...
  const links = db.prepare(`
    ...
    ${isAuth ? '' : 'WHERE l.is_private = 0 AND (c.id IS NULL OR c.is_private = 0)'}
...
router.post('/', authMiddleware, (req, res) => {
  const { title, url, description = '', icon = '', category_id, tags = [], sort_order = 0, is_private = 0 } = req.body;
  if (!title || !url) {
  ...
  const stmt = db.prepare(`
    INSERT INTO links (title, url, description, icon, category_id, tags, sort_order, is_private)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
  `);
  const result = stmt.run(title, url, description, icon, category_id || null, JSON.stringify(tags), sort_order, is_private ? 1 : 0);
...
router.put('/:id', authMiddleware, (req, res) => {
  const { id } = req.params;
  const { title, url, description, icon, category_id, tags, sort_order, is_private } = req.body;
  ...
  `).run(updated.title, updated.url, updated.description, updated.icon,
         updated.category_id, updated.tags, updated.sort_order, updated.is_private, id);
